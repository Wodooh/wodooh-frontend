...
| Auth (`/login`) | `app/login/page.tsx` | `app/nexus.css` + `app/login/login.css` (`nx-login-*` layout helpers) | reads + sets `[data-nx-theme]` |
...
**Auth shell (login)** is a centered card on the page background with a theme toggle in the top-right corner. The card is `max-width: 380px`, capped by `calc(100vw - 32px)` on narrow viewports. It uses the same `--nx-*` tokens as the rest of the app — no parallel palette, no exception. Layout helpers (`.nx-login-shell`, `.nx-login-card`, `.nx-login-input-wrap`, `.nx-login-submit`, etc.) live in `app/login/login.css` and only handle page-level structure (centered column, taller 40 px input wrap for readability, branded logo tile, bottom legal-link footer); colors, radii, shadows, and typography all come from `app/nexus.css`.

The login imports both `../nexus.css` (the canonical token surface) and `./login.css` (layout helpers). Don't fork the `nx-input-wrap` / `nx-btn-primary` definitions — `login.css` extends them, never replaces them.
